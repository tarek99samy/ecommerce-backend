...
const createSchema = joi.object({
  ...
  entity_type: joi.string().required().valid('comment', 'message').messages({
    'string.base': 'Entity type must be a string',
    'any.only': 'Entity type must be comment or message',
...
const editSchema = joi.object({
  ...
  entity_type: joi.string().optional().valid('comment', 'message').messages({
    'string.base': 'Entity type must be a string',
    'any.only': 'Entity type must be comment or message'
  }),
  user_id: joi.number().optional().min(1).messages({
